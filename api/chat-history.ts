...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  //   fields by api_name. Groups fields per spec section 9.1–9.11.
  //   Returns: created[], skipped[], failed[] with details.
  if (req.method === "GET" && req.query.action === "asbl-spec-create-fields") {
    const incomingSecret = (req.query.secret as string) || "";
    const expectedSecret = process.env.INHOUSE_POSTHOOK_SECRET || "";
    if (!expectedSecret || incomingSecret !== expectedSecret) {
      return res.status(401).json({ error: "Unauthorized — pass ?secret=<INHOUSE_POSTHOOK_SECRET>" });
    ...
    try {
      ...
      // Fetch existing fields first (idempotency)
      ...
      // ───── Spec Section 9 — full field catalog ─────
      // Format: { api: <api_name>, spec: <Zoho metadata spec> }
      const desired: Array<{ section: string; api: string; spec: any }> = [
        // 9.1 Identity & Attribution
        { section: "9.1", api: "Lead_Source_Detail", spec: { field_label: "ASBL Loft Spec — Source Detail", data_type: "picklist", pick_list_values: pickList(["Website Chatbot", "WhatsApp Direct", "Manual Entry", "FIM Forms", "Channel Partner"]) } },
        { section: "9.1", api: "Visitor_ID", spec: { field_label: "ASBL Loft Spec — Visitor ID", data_type: "text", length: 100 } },
        { section: "9.1", api: "Session_ID", spec: { field_label: "ASBL Loft Spec — Origin Session ID", data_type: "text", length: 100 } },
        { section: "9.1", api: "GCLID", spec: { field_label: "ASBL Loft Spec — Google Click ID", data_type: "text", length: 255 } },
        { section: "9.1", api: "FBCLID", spec: { field_label: "ASBL Loft Spec — Meta Click ID", data_type: "text", length: 255 } },
        { section: "9.1", api: "WBRAID", spec: { field_label: "ASBL Loft Spec — Web-to-app GAds ID", data_type: "text", length: 255 } },
        { section: "9.1", api: "GBRAID", spec: { field_label: "ASBL Loft Spec — iOS GAds ID", data_type: "text", length: 255 } },
        { section: "9.1", api: "Landing_Page", spec: { field_label: "ASBL Loft Spec — Landing Page URL", data_type: "website" } },
        { section: "9.1", api: "Referrer_Domain", spec: { field_label: "ASBL Loft Spec — Referrer Domain", data_type: "text", length: 200 } },
        { section: "9.1", api: "Device_Fingerprint", spec: { field_label: "ASBL Loft Spec — Device Fingerprint", data_type: "text", length: 200 } },
        { section: "9.1", api: "IP_City", spec: { field_label: "ASBL Loft Spec — IP City", data_type: "text", length: 80 } },
        { section: "9.1", api: "IP_Country", spec: { field_label: "ASBL Loft Spec — IP Country", data_type: "text", length: 10 } },
        { section: "9.1", api: "First_Seen_At", spec: { field_label: "ASBL Loft Spec — First Seen", data_type: "datetime" } },

        // 9.2 Stage & Status (THE CORE STATE MACHINE)
        { section: "9.2", api: "Stage", spec: { field_label: "ASBL Loft Spec — Stage", data_type: "picklist", pick_list_values: pickList(["New Lead", "In Conversation", "Site Visit Scheduled", "Site Visit Done", "Booked", "Closed"]) } },
        { section: "9.2", api: "Status", spec: { field_label: "ASBL Loft Spec — Status", data_type: "picklist", pick_list_values: pickList(["Outreach Pending", "Talking", "Awaiting Reply", "No Response", "Callback Pending"]) } },
        { section: "9.2", api: "Callback_Source", spec: { field_label: "ASBL Loft Spec — Callback Source", data_type: "picklist", pick_list_values: pickList(["Customer Requested", "System Scheduled"]) } },
        { section: "9.2", api: "Closure_Reason", spec: { field_label: "ASBL Loft Spec — Closure Reason", data_type: "picklist", pick_list_values: pickList(["Not Interested", "Unreachable", "Wrong Number", "Broker", "Visit No-Show", "Opted Out", "Disqualified"]) } },
        { section: "9.2", api: "Stage_Updated_At", spec: { field_label: "ASBL Loft Spec — Stage Updated At", data_type: "datetime" } },
        { section: "9.2", api: "Status_Updated_At", spec: { field_label: "ASBL Loft Spec — Status Updated At", data_type: "datetime" } },
        { section: "9.2", api: "Last_State_Change_Source", spec: { field_label: "ASBL Loft Spec — State Source", data_type: "picklist", pick_list_values: pickList(["whatsapp_bot", "voice_bot", "chatbot", "system", "vendor_webhook", "manual"]) } },
        { section: "9.2", api: "Last_State_Change_Reason", spec: { field_label: "ASBL Loft Spec — State Reason", data_type: "text", length: 200 } },

        // 9.3 WhatsApp Channel
        { section: "9.3", api: "WhatsApp_State", spec: { field_label: "ASBL Loft Spec — WhatsApp State", data_type: "picklist", pick_list_values: pickList(["Not Started", "Template Sent", "Delivered", "Read", "Replied", "Bot Active", "Awaiting Reply", "Stalled 24h", "Opted Out"]) } },
        { section: "9.3", api: "WhatsApp_Last_Activity_At", spec: { field_label: "ASBL Loft Spec — WhatsApp Last Activity", data_type: "datetime" } },
        { section: "9.3", api: "WhatsApp_Template_Last_Sent", spec: { field_label: "ASBL Loft Spec — Last Template Sent", data_type: "text", length: 100 } },
        { section: "9.3", api: "WhatsApp_Reply_Count", spec: { field_label: "ASBL Loft Spec — WhatsApp Reply Count", data_type: "integer" } },
        { section: "9.3", api: "WhatsApp_Service_Window_Expires_At", spec: { field_label: "ASBL Loft Spec — WA 24h Window Expires", data_type: "datetime" } },
        { section: "9.3", api: "Opt_Out_WhatsApp", spec: { field_label: "ASBL Loft Spec — Opted Out of WhatsApp", data_type: "boolean" } },

        // 9.4 Voice Channel
        { section: "9.4", api: "Call_State", spec: { field_label: "ASBL Loft Spec — Call State", data_type: "picklist", pick_list_values: pickList(["Not Started", "Dialing", "Connected", "In Progress", "Voicemail", "No Answer", "Disconnected", "Completed", "DNC"]) } },
        { section: "9.4", api: "Call_Last_Attempt_At", spec: { field_label: "ASBL Loft Spec — Last Call Attempt", data_type: "datetime" } },
        { section: "9.4", api: "Call_Connected_Count", spec: { field_label: "ASBL Loft Spec — Calls Connected", data_type: "integer" } },
        { section: "9.4", api: "Call_Last_Disposition", spec: { field_label: "ASBL Loft Spec — Last Call Disposition", data_type: "text", length: 200 } },
        { section: "9.4", api: "Preferred_Callback_Time", spec: { field_label: "ASBL Loft Spec — Preferred Callback Time", data_type: "datetime" } },
        { section: "9.4", api: "Opt_Out_Calls", spec: { field_label: "ASBL Loft Spec — Do Not Call", data_type: "boolean" } },

        // 9.5 Bot State
        { section: "9.5", api: "Bot_Turn_Count", spec: { field_label: "ASBL Loft Spec — Bot Turn Count", data_type: "integer" } },
        { section: "9.5", api: "Bot_Last_Intent", spec: { field_label: "ASBL Loft Spec — Bot Intent Tag", data_type: "picklist", pick_list_values: pickList(["pricing", "amenities", "floor_plan", "location", "timeline", "objection", "book_visit", "other"]) } },
        { section: "9.5", api: "Bot_Last_Confidence", spec: { field_label: "ASBL Loft Spec — Last Intent Confidence", data_type: "double" } },
        { section: "9.5", api: "Active_Channel", spec: { field_label: "ASBL Loft Spec — Active Channel", data_type: "picklist", pick_list_values: pickList(["WhatsApp", "Voice", "Web", "None"]) } },
        { section: "9.5", api: "LLM_Cost_Total_Micros", spec: { field_label: "ASBL Loft Spec — LLM Cost (micros)", data_type: "integer" } },
        { section: "9.5", api: "Bot_Last_Cost_Path", spec: { field_label: "ASBL Loft Spec — Last Classifier Path", data_type: "picklist", pick_list_values: pickList(["rules", "cache", "llm"]) } },

        // 9.6 Slots (filled by AI conversation layer)
        { section: "9.6", api: "Intent", spec: { field_label: "ASBL Loft Spec — Intent", data_type: "picklist", pick_list_values: pickList(["End Use", "Investment", "NRI", "Upgrade", "Exploring"]) } },
        { section: "9.6", api: "Budget_Bucket", spec: { field_label: "ASBL Loft Spec — Budget Range Bucket", data_type: "picklist", pick_list_values: pickList(["under_1_5cr", "1_5_to_2cr", "2_to_2_5cr", "above_2_5cr", "undisclosed"]) } },
        { section: "9.6", api: "Timeline_Bucket", spec: { field_label: "ASBL Loft Spec — Timeline", data_type: "picklist", pick_list_values: pickList(["immediate", "1_3_months", "3_6_months", "exploring"]) } },
        { section: "9.6", api: "Configuration_Interest", spec: { field_label: "ASBL Loft Spec — Configuration Interest", data_type: "picklist", pick_list_values: pickList(["3bhk_1695", "3bhk_1870", "either", "undecided"]) } },
        { section: "9.6", api: "Current_Location", spec: { field_label: "ASBL Loft Spec — Current Location", data_type: "text", length: 100 } },
        { section: "9.6", api: "Workplace_Area", spec: { field_label: "ASBL Loft Spec — Workplace Area", data_type: "text", length: 100 } },
        { section: "9.6", api: "Loan_Required", spec: { field_label: "ASBL Loft Spec — Loan Required", data_type: "picklist", pick_list_values: pickList(["yes", "no", "unsure"]) } },
        { section: "9.6", api: "Competitor_Projects", spec: { field_label: "ASBL Loft Spec — Competitor Projects", data_type: "multiselectpicklist", pick_list_values: pickList(["My Home", "Aparna", "Rajapushpa", "Prestige", "Sumadhura", "Lodha", "DLF", "Brigade", "Other"]) } },
        { section: "9.6", api: "Slot_Last_Updated_At", spec: { field_label: "ASBL Loft Spec — Slots Last Updated", data_type: "datetime" } },

        // 9.7 Lead Score
        { section: "9.7", api: "Lead_Score", spec: { field_label: "ASBL Loft Spec — Qualification Score", data_type: "integer" } },
        { section: "9.7", api: "Lead_Tier", spec: { field_label: "ASBL Loft Spec — Qualification Tier", data_type: "picklist", pick_list_values: pickList(["Hot", "Warm", "Cold", "Disqualified"]) } },
        { section: "9.7", api: "Score_Last_Computed_At", spec: { field_label: "ASBL Loft Spec — Score Updated At", data_type: "datetime" } },
        { section: "9.7", api: "Predicted_Value_INR", spec: { field_label: "ASBL Loft Spec — Predicted Value", data_type: "currency" } },

        // 9.8 Site Visit
        { section: "9.8", api: "Site_Visit_Date", spec: { field_label: "ASBL Loft Spec — Visit Date", data_type: "date" } },
        { section: "9.8", api: "Site_Visit_Slot", spec: { field_label: "ASBL Loft Spec — Visit Slot", data_type: "text", length: 40 } },
        { section: "9.8", api: "Site_Visit_Confirmation_Source", spec: { field_label: "ASBL Loft Spec — Confirmed Via", data_type: "picklist", pick_list_values: pickList(["WhatsApp", "Voice", "Web"]) } },
        { section: "9.8", api: "Site_Visit_Reminder_24h_Sent", spec: { field_label: "ASBL Loft Spec — Reminder 24h Sent", data_type: "boolean" } },
        { section: "9.8", api: "Site_Visit_Reminder_3h_Sent", spec: { field_label: "ASBL Loft Spec — Reminder 3h Sent", data_type: "boolean" } },
        { section: "9.8", api: "Site_Visit_Attended", spec: { field_label: "ASBL Loft Spec — Visit Attended", data_type: "boolean" } },
        { section: "9.8", api: "Site_Visit_Attended_At", spec: { field_label: "ASBL Loft Spec — Visit Attended At", data_type: "datetime" } },

        // 9.9 Outcome
        { section: "9.9", api: "Booking_Token_Paid", spec: { field_label: "ASBL Loft Spec — Token Paid", data_type: "boolean" } },
        { section: "9.9", api: "Booking_Amount", spec: { field_label: "ASBL Loft Spec — Booking Amount", data_type: "currency" } },
        { section: "9.9", api: "Booking_Date", spec: { field_label: "ASBL Loft Spec — Booking Date", data_type: "date" } },
        { section: "9.9", api: "Closed_At", spec: { field_label: "ASBL Loft Spec — Closed At", data_type: "datetime" } },

        // 9.10 Progress Flags (16 booleans for fast view filtering)
        { section: "9.10", api: "F_WA_Template_Sent", spec: { field_label: "ASBL Loft Spec — WA Template Sent Flag", data_type: "boolean" } },
        { section: "9.10", api: "F_WA_Replied", spec: { field_label: "ASBL Loft Spec — WA Replied Flag", data_type: "boolean" } },
        { section: "9.10", api: "F_Call_Attempted", spec: { field_label: "ASBL Loft Spec — Call Attempted Flag", data_type: "boolean" } },
        { section: "9.10", api: "F_Call_Connected", spec: { field_label: "ASBL Loft Spec — Call Connected Flag", data_type: "boolean" } },
        { section: "9.10", api: "F_Intent_Captured", spec: { field_label: "ASBL Loft Spec — Intent Captured Flag", data_type: "boolean" } },
        { section: "9.10", api: "F_Budget_Captured", spec: { field_label: "ASBL Loft Spec — Budget Captured Flag", data_type: "boolean" } },
        { section: "9.10", api: "F_Timeline_Captured", spec: { field_label: "ASBL Loft Spec — Timeline Captured Flag", data_type: "boolean" } },
        { section: "9.10", api: "F_Configuration_Captured", spec: { field_label: "ASBL Loft Spec — Configuration Captured Flag", data_type: "boolean" } },
        { section: "9.10", api: "F_Site_Visit_Offered", spec: { field_label: "ASBL Loft Spec — Visit Offered Flag", data_type: "boolean" } },
        { section: "9.10", api: "F_Site_Visit_Slot_Proposed", spec: { field_label: "ASBL Loft Spec — Slot Proposed Flag", data_type: "boolean" } },
        { section: "9.10", api: "F_Site_Visit_Confirmed", spec: { field_label: "ASBL Loft Spec — Visit Confirmed Flag", data_type: "boolean" } },
        { section: "9.10", api: "F_Site_Visit_Attended", spec: { field_label: "ASBL Loft Spec — Visit Attended Flag", data_type: "boolean" } },
        { section: "9.10", api: "F_Token_Paid", spec: { field_label: "ASBL Loft Spec — Token Paid Flag", data_type: "boolean" } },
        { section: "9.10", api: "F_Marked_Not_Interested", spec: { field_label: "ASBL Loft Spec — Not Interested Flag", data_type: "boolean" } },
        { section: "9.10", api: "F_Marked_Broker", spec: { field_label: "ASBL Loft Spec — Broker Suspected Flag", data_type: "boolean" } },
        { section: "9.10", api: "F_Opt_Out_All", spec: { field_label: "ASBL Loft Spec — Opted Out All Flag", data_type: "boolean" } },

        // 9.11 System / Audit
        { section: "9.11", api: "Last_Inbound_At", spec: { field_label: "ASBL Loft Spec — Last Inbound", data_type: "datetime" } },
        { section: "9.11", api: "Next_Action_At", spec: { field_label: "ASBL Loft Spec — Next Action At", data_type: "datetime" } },
        { section: "9.11", api: "Next_Action_Type", spec: { field_label: "ASBL Loft Spec — Next Action Type", data_type: "picklist", pick_list_values: pickList(["send_wa_template", "send_wa_message", "dial", "send_reminder", "no_action"]) } },
        { section: "9.11", api: "Active_Cadence", spec: { field_label: "ASBL Loft Spec — Active Cadence", data_type: "picklist", pick_list_values: pickList(["outreach", "recovery", "cooled", "reminder", "decision_followup", "none"]) } },
        { section: "9.11", api: "Cadence_Step", spec: { field_label: "ASBL Loft Spec — Cadence Step", data_type: "integer" } },
        { section: "9.11", api: "Lead_Locked_For_Audit", spec: { field_label: "ASBL Loft Spec — Audit Lock", data_type: "boolean" } },
      ];

      const toCreate = desired.filter((d) => !existingApis.has(d.api));
      const alreadyExist = desired.filter((d) => existingApis.has(d.api));

      // Create in batches of 1 to get clean per-field error reporting.
      const results: any[] = [];
      ...
      const created = results.filter((r) => r.success).map((r) => r.api_name);
      const failed = results.filter((r) => !r.success);

      // Group summary by section
      const summary: Record<string, { spec: number; existed: number; created: number; failed: number }> = {};
      for (const d of desired) {
        if (!summary[d.section]) summary[d.section] = { spec: 0, existed: 0, created: 0, failed: 0 };
        summary[d.section].spec++;
        if (existingApis.has(d.api)) summary[d.section].existed++;
        else if (created.includes(d.api)) summary[d.section].created++;
        else summary[d.section].failed++;
      }

      return res.status(200).json({
        status: failed.length === 0 ? "ok" : "partial",
        totals: {
          spec_fields_total: desired.length,
          already_existed: alreadyExist.length,
          created: created.length,
          failed: failed.length,
        },
        per_section_summary: summary,
        already_existed: alreadyExist.map((d) => d.api),
        created,
        failed,
      });
    } catch (err: any) {
